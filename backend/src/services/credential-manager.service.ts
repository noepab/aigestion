...
export class CredentialManagerService {
  ...
  async verifyGoogleCloud(): Promise<CredentialStatus> {
    ...
    if (!hasCreds) {return this.report('Google Cloud', 'missing');}

    try {
      // Try to list a single secret or access metadata
      const projectId = process.env.GOOGLE_CLOUD_PROJECT_ID;
      if (!projectId) {return this.report('Google Cloud', 'invalid', 'Missing Project ID');}
      return this.report('Google Cloud', 'valid');
    } catch (e) {
      return this.report('Google Cloud', 'invalid', (e as any).message);
    }
  }

  async verifyGemini(): Promise<CredentialStatus> {
    const key = process.env.GEMINI_API_KEY || process.env.GENAI_API_KEY;
    if (!key) {return this.report('Gemini AI', 'missing');}
    ...
    try {
      ...
      return this.report('Gemini AI', 'invalid', (e as any).message);
    }
  }

  async verifyStripe(): Promise<CredentialStatus> {
    const key = process.env.STRIPE_SECRET_KEY;
    if (!key) {return this.report('Stripe', 'missing');}
    ...
    try {
      ...
      return this.report('Stripe', 'invalid', (e as any).message);
    }
  }

  async verifyTelegram(): Promise<CredentialStatus> {
    const token = process.env.TELEGRAM_BOT_TOKEN;
    if (!token) {return this.report('Telegram', 'missing');}
    ...
    try {
      ...
      return this.report('Telegram', 'invalid', (e as any).message);
    }
  }

  async verifyInstagram(): Promise<CredentialStatus> {
    const token = process.env.INSTAGRAM_ACCESS_TOKEN;
    if (!token) {return this.report('Instagram', 'missing');}

    try {
      await axios.get(`https://graph.facebook.com/v19.0/me?access_token=${token}`);
      return this.report('Instagram', 'valid');
    } catch (e) {
      return this.report('Instagram', 'invalid', (e as any).message);
    }
  }

  private report(provider: string, status: CredentialStatus['status'], message?: string): CredentialStatus {
    return {
      provider,
      status,
      message,
