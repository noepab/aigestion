...
export async function getRecentLogs(req: Request, res: Response) {
  try {
    ...
    (res as any).status(500).json(buildError('Failed to fetch logs', 'LOGS_ERROR', 500, (req as any).requestId));
  }
}
...
export async function clearLogs(req: Request, res: Response) {
  try {
    ...
    (res as any).status(500).json(buildError('Failed to clear logs', 'LOGS_ERROR', 500, (req as any).requestId));
  }
}
